...
def build_cases_from_json(
...
) -> tuple[Path, list[ImplCase]]:
    data = load_json(regression_json, "implementation regression JSON")
    ...
    default_flow_params = normalize_dict(
    ...
    default_rtl_defines.update(normalize_dict(defaults.get("impl_defines", {}), "defaults.impl_defines"))

    default_auto_defines = bool(defaults.get("auto_defines", True))

    raw_cases = data.get("cases")
    if raw_cases is None:
        raise ValueError("[ERROR] implementation regression JSON missing required field: cases")
    ...
    for idx, raw_case in enumerate(raw_cases):
        ...
        if case_name in seen:
            raise ValueError(f"[ERROR] duplicated implementation regression case name: {case_name}")
        ...
        rtl_defines.update(normalize_dict(raw_case.get("impl_defines", {}), f"{case_name}.impl_defines"))
        ...
        case_dir = regression_dir / case_name / "implementation"
    ...
    if not cases:
        raise RuntimeError("[ERROR] no enabled implementation regression cases found")
...
def run_impl_case(
    ...
    run_impl_tcl = resolve_tcl_from_env(
        "RUN_IMPL_TCL",
        project_root,
        ["scripts/tcl/run_impl.tcl", "scripts/run_impl.tcl"],
    )

    if not run_impl_tcl.exists():
        raise FileNotFoundError(f"[ERROR] run_impl.tcl not found: {run_impl_tcl}")
...
def collect_impl_metrics(case: ImplCase) -> dict[str, Any]:
    reports_dir = case.case_dir / "reports"

    metrics_rpt = reports_dir / "impl_metrics.rpt"
    utilization_rpt = reports_dir / "utilization_impl.rpt"
    timing_summary_rpt = reports_dir / "timing_summary_impl.rpt"
    route_status_rpt = reports_dir / "route_status_impl.rpt"
    ...
    metrics.setdefault("CARRY8", None)
    metrics.setdefault("URAM", None)
    metrics.setdefault("BUFG", None)
    ...
    metrics["route_status_report"] = route_status_rpt

    metrics["synth_checkpoint"] = case.case_dir / "checkpoints" / "synth.dcp"
    metrics["opt_checkpoint"] = case.case_dir / "checkpoints" / "opt.dcp"
    metrics["placed_checkpoint"] = case.case_dir / "checkpoints" / "placed.dcp"
    metrics["routed_checkpoint"] = case.case_dir / "checkpoints" / "routed.dcp"

    return metrics


def evaluate_impl_result(metrics: dict[str, Any], limits: dict[str, Any]) -> tuple[str, str]:
    messages: list[str] = []

    min_wns = as_float(limits.get("MIN_WNS_NS", 0.0))
    min_whs = as_float(limits.get("MIN_WHS_NS", 0.0))

    wns = as_float(metrics.get("WNS_NS"))
    whs = as_float(metrics.get("WHS_NS"))
    ...
    if whs is None:
        messages.append("missing WHS")
    elif min_whs is not None and whs < min_whs:
        messages.append(f"WHS {whs:.3f} ns < required {min_whs:.3f} ns")
...
def write_impl_summary_report(
    ...
    report_file = case.case_dir / "reports" / "impl_summary.rpt"
    report_file.parent.mkdir(parents=True, exist_ok=True)

    with report_file.open("w") as f:
        f.write("IMPLEMENTATION REGRESSION REPORT\n")
        f.write("================================\n\n")
        ...
        if case.limits:
            ...
            f.write(f"{'MIN_WNS_NS':<24}: 0.0  # implicit default\n")
            f.write(f"{'MIN_WHS_NS':<24}: 0.0  # implicit default\n")
        ...
        f.write(f"{'CARRY8':<24}: {metrics.get('CARRY8')}\n")
        f.write(f"{'DSP':<24}: {metrics.get('DSP')}\n")
        f.write(f"{'BRAM':<24}: {metrics.get('BRAM')}\n")
        f.write(f"{'URAM':<24}: {metrics.get('URAM')}\n")
        f.write(f"{'BUFG':<24}: {metrics.get('BUFG')}\n")
        ...
        f.write(f"{'route_status_report':<24}: {metrics.get('route_status_report')}\n")
        f.write(f"{'metrics_report':<24}: {metrics.get('metrics_report')}\n")
        f.write(f"{'synth_checkpoint':<24}: {metrics.get('synth_checkpoint')}\n")
        f.write(f"{'opt_checkpoint':<24}: {metrics.get('opt_checkpoint')}\n")
        f.write(f"{'placed_checkpoint':<24}: {metrics.get('placed_checkpoint')}\n")
        f.write(f"{'routed_checkpoint':<24}: {metrics.get('routed_checkpoint')}\n")
...
def write_case_manifest(
    ...
    data = {
        ...
        "artifacts": {
            ...
            "route_status_report": metrics.get("route_status_report"),
            "metrics_report": metrics.get("metrics_report"),
            "synth_checkpoint": metrics.get("synth_checkpoint"),
            "opt_checkpoint": metrics.get("opt_checkpoint"),
            "placed_checkpoint": metrics.get("placed_checkpoint"),
            "routed_checkpoint": metrics.get("routed_checkpoint"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="run_regression_impl",
        description="Run implementation regressions for the current module.",
...
def print_case_info(case: ImplCase) -> None:
    section(f"IMPL CASE {case.name}")
...
def print_results_summary(
    ...
    banner("IMPLEMENTATION REGRESSION SUMMARY")
    ...
    print("")
    print(color("  Per-case implementation results", C.BOLD))
    print(color("  " + "-" * 148, C.GRAY))
    ...
    for result in results:
        ...
        print(
            f"  {status} "
            f"{result.name:<38} "
            f"{fmt_float(result.clock_period_ns):>7} "
            f"{fmt_float(result.wns_ns):>8} "
            f"{fmt_float(result.whs_ns):>8} "
            f"{fmt_float(result.est_fmax_mhz):>9} "
            f"{fmt_any(result.lut):>6} "
            f"{fmt_any(result.ff):>6} "
            f"{fmt_any(result.carry8):>7} "
            f"{fmt_any(result.dsp):>5} "
            f"{fmt_any(result.bram):>6} "
            f"{fmt_any(result.uram):>5} "
            f"{fmt_any(result.bufg):>5} "
            f"{format_seconds(result.elapsed_s):>9}"
        )
    ...
    if failed == 0:
        pass_msg("implementation regression completed successfully")
    else:
        fail_msg("implementation regression failed")
...
def main() -> int:
    ...
    regression_json_env = os.environ.get(f"{prefix}_IMPL_REGRESSION_JSON")
    if regression_json_env:
        regression_json = resolve_path(regression_json_env, module_dir)
    else:
        regression_json = module_dir / "implementation" / f"{module_name}_impl_regression.json"
    ...
    if args.case:
        ...
        if missing:
            raise ValueError(f"[ERROR] unknown implementation regression case(s): {sorted(missing)}")

        cases = [case for case in cases if case.name in selected]

    if args.list_cases:
        banner("AVAILABLE IMPLEMENTATION REGRESSION CASES")
    ...
    if not cases:
        raise RuntimeError("[ERROR] no implementation regression cases selected")

    banner("IMPLEMENTATION REGRESSION")
    ...
    if args.skip_impl:
        warn_msg("Vivado implementation disabled (--skip-impl)")
    ...
    for case in cases:
        ...
        try:
            ...
            metrics = collect_impl_metrics(case)
            status, message = evaluate_impl_result(metrics, case.limits)

            case_elapsed = time.perf_counter() - case_t0

            summary_report = write_impl_summary_report(
                case=case,
                metrics=metrics,
                status=status,
                message=message,
            ...
            step_msg(f"implementation summary: {summary_report}")
            ...
            result = ImplResult(
                name=case.name,
                status=status,
                clock_period_ns=as_float(metrics.get("CLOCK_PERIOD_NS")),
                wns_ns=as_float(metrics.get("WNS_NS")),
                whs_ns=as_float(metrics.get("WHS_NS")),
                est_fmax_mhz=as_float(metrics.get("EST_FMAX_MHZ")),
                lut=as_int(metrics.get("LUT")),
                ff=as_int(metrics.get("FF")),
                carry8=as_int(metrics.get("CARRY8")),
                dsp=as_int(metrics.get("DSP")),
                bram=as_float(metrics.get("BRAM")),
                uram=as_int(metrics.get("URAM")),
                bufg=as_int(metrics.get("BUFG")),
                elapsed_s=case_elapsed,
                message=message,
                manifest_file=manifest_file,
                summary_report=summary_report,
            ...
            try:
                ...
                warn_msg(f"could not write implementation reports for {case.name}: {report_exc}")
