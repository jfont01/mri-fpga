...
//
// Corre el golden C++ (cmul_model) y vuelca con VectorTraceFiles:
//
...
//
// El testbench SystemVerilog (cmul_tb.sv) reinyecta esos estimulos en el RTL
...
using raw_in_t = ap_int<cmul::NB_IN>;
...
raw_in_t max_raw()
{
    ...
    for (int i = 0; i < cmul::NB_IN - 1; ++i) {
        value[i] = 1;
    }

    value[cmul::NB_IN - 1] = 0;

    return value;
}

cmul::in_t raw_to_input(const raw_in_t& raw)
{
    cmul::in_t value;
    value.range(cmul::NB_IN - 1, 0) = raw.range(cmul::NB_IN - 1, 0);
    return value;
}
...
void run_tb(const tb_args& args)
{
    ...
    report_file << "----------\n";
    report_file << "NB_IN       : " << cmul::NB_IN   << "\n";
    report_file << "NBF_IN      : " << cmul::NBF_IN  << "\n";
    report_file << "NB_OUT      : " << cmul::NB_OUT  << "\n";
    report_file << "NBF_OUT     : " << cmul::NBF_OUT << "\n";
    report_file << "NB_FULL     : " << cmul::NB_FULL << "\n";
    report_file << "\n";
...
{
    try {
        const auto args = cmul_tb::parse_args(argc, argv);
        cmul_tb::run_tb(args);
        return 0;
    }
    catch (const std::exception& e) {
        std::cerr << "[cmul_tb] ERROR: " << e.what() << "\n";
        return 1;
    }
}