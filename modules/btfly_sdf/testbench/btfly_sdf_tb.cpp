...
{
    try {
        const auto args = btfly_sdf_tb::parse_args(argc, argv);
        btfly_sdf_tb::run_tb(args);
        return 0;
    }
    catch (const std::exception& e) {
        std::cerr << "[btfly_sdf_tb] ERROR: " << e.what() << "\n";
        return 1;
    }
}
