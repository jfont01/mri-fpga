...
{
    try {
        const auto args = fft1d_tb::parse_args(argc, argv);
        fft1d_tb::run_tb(args);
        return 0;
    }
    catch (const std::exception& e) {
        std::cerr << "[fft1d_tb] ERROR: " << e.what() << "\n";
        return 1;
    }
}
