...
void run_tb(const tb_args& args)
{
    ...
    rtl::ClockDomain clk;
    fft1d_r2_sdf::fft1d_r2_sdf_model dut;
    ...
    report_file << "==========\n\n";
    report_file << "module      : fft1d_r2_sdf\n";
...
{
    try {
        const auto args = fft1d_r2_sdf_tb::parse_args(argc, argv);
        fft1d_r2_sdf_tb::run_tb(args);
        return 0;
    }
    catch (const std::exception& e) {
        std::cerr << "[fft1d_r2_sdf_tb] ERROR: " << e.what() << "\n";
        return 1;
    }
}
