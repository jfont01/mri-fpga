...
void run_tb(const tb_args& args)
{
    ...
    rtl::ClockDomain clk;
    tile_transpose::tile_transpose_model dut;
    ...
    report_file << "==========\n\n";
    report_file << "module      : tile_transpose\n";
...
{
    try {
        const auto args = tile_transpose_tb::parse_args(argc, argv);
        tile_transpose_tb::run_tb(args);
        return 0;
    }
    catch (const std::exception& e) {
        std::cerr << "[tile_transpose_tb] ERROR: " << e.what() << "\n";
        return 1;
    }
}
