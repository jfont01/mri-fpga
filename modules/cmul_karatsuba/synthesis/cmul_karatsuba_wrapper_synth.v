...
`ifndef CMUL_KARATSUBA_WRAPPER_SYNTH_V
`define CMUL_KARATSUBA_WRAPPER_SYNTH_V
...
// -----------------------------------------------------------------------------
// cmul_karatsuba_wrapper_synth
//
// cmul_karatsuba es COMBINACIONAL: no tiene reloj propio. Para que la sintesis mida algo
...
//
//   puertos -> registros de entrada -> cmul_karatsuba -> registros de salida -> puertos
//
// Con esa estructura el UNICO camino sincronico es
//
//   FF -> cmul_karatsuba (logica combinacional) -> FF
//
...
// -----------------------------------------------------------------------------
module cmul_karatsuba_wrapper_synth #(
    parameter int NB_IN   = `CMUL_KARATSUBA_NB_IN,
    parameter int NBF_IN  = `CMUL_KARATSUBA_NBF_IN,
    parameter int NB_OUT  = `CMUL_KARATSUBA_NB_OUT,
    parameter int NBF_OUT = `CMUL_KARATSUBA_NBF_OUT
)(
    input  wire                     i_clock,
