...
module cmul_karatsuba_tb;

    localparam int NB_IN   = `CMUL_KARATSUBA_NB_IN;
    localparam int NBF_IN  = `CMUL_KARATSUBA_NBF_IN;
    localparam int NB_OUT  = `CMUL_KARATSUBA_NB_OUT;
    localparam int NBF_OUT = `CMUL_KARATSUBA_NBF_OUT;
    ...
    initial begin
        if (!$value$plusargs("CASE_DIR=%s", case_dir))
            $fatal(1, "[cmul_karatsuba_tb] falta plusarg: +CASE_DIR=<path>");
        if (!$value$plusargs("N_CYCLES=%d", n_cycles))
            $fatal(1, "[cmul_karatsuba_tb] falta plusarg: +N_CYCLES=<value>");
        if (n_cycles < 0)
            $fatal(1, "[cmul_karatsuba_tb] N_CYCLES invalido=%0d", n_cycles);
        ...
        if (fd_i1r==0 || fd_i1i==0 || fd_i2r==0 || fd_i2i==0)
            $fatal(1, "[cmul_karatsuba_tb] no pude abrir algun .dat de estimulo en %s", in_dir);

        fd_ore = $fopen(p_o_re, "w");
        fd_oim = $fopen(p_o_im, "w");
        fd_csv = $fopen(act_csv_path, "w");
        if (fd_ore==0 || fd_oim==0 || fd_csv==0)
            $fatal(1, "[cmul_karatsuba_tb] no pude abrir salidas en %s", act_out_dir);
        ...
        if ($test$plusargs("WAVES")) begin
            ...
            $dumpfile(waves_path);
            $dumpvars(0, cmul_karatsuba_tb);
            $display("[cmul_karatsuba_tb] waves: %s", waves_path);
        end

        $display("[cmul_karatsuba_tb] CASE_DIR = %s", case_dir);
        $display("[cmul_karatsuba_tb] N_CYCLES = %0d", n_cycles);
        $display("[cmul_karatsuba_tb] NB_IN=%0d NBF_IN=%0d NB_OUT=%0d NBF_OUT=%0d",
        ...
        for (int cycle = 0; cycle < n_cycles; cycle++) begin
            if ($fscanf(fd_i1r, "%d", s_i1r) != 1)
                $fatal(1, "[cmul_karatsuba_tb] i_1_re: faltan muestras en cycle=%0d", cycle);
            if ($fscanf(fd_i1i, "%d", s_i1i) != 1)
                $fatal(1, "[cmul_karatsuba_tb] i_1_im: faltan muestras en cycle=%0d", cycle);
            if ($fscanf(fd_i2r, "%d", s_i2r) != 1)
                $fatal(1, "[cmul_karatsuba_tb] i_2_re: faltan muestras en cycle=%0d", cycle);
            if ($fscanf(fd_i2i, "%d", s_i2i) != 1)
                $fatal(1, "[cmul_karatsuba_tb] i_2_im: faltan muestras en cycle=%0d", cycle);
        ...
        $display("[cmul_karatsuba_tb] simulacion completa");
        $finish;
    end
