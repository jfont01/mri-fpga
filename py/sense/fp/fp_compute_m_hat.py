...
from fp_cholesky_LLH import fp_cholesky_llh
from fp_cholesky_LDLH import fp_cholesky_ldlh


def fp_compute_m_hat(A, b, compute_type, cholesky_type=None):
    if A.shape[0:2] != (2, 2):
        ...
        for ny_alias in range(offset):
            ...
            if compute_type == "numpy-linalg-solve":
                ...
                if cholesky_type == "LLH":
                    m_hat[:, nx, ny_alias] = fp_cholesky_llh(Aij, bi)
                elif cholesky_type == "LDLH":
                    m_hat[:, nx, ny_alias] = fp_cholesky_ldlh(Aij, bi)
                else:
