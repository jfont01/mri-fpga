...
def save_A_compare_figures(
    A_fp: np.ndarray,
    A_fxp: np.ndarray,
    out_dir: str,
    prefix: str = "A"
) -> None:
    if A_fp.shape != A_fxp.shape:
        raise ValueError(f"Shape mismatch: A_fp={A_fp.shape}, A_fxp={A_fxp.shape}")

    if A_fp.shape[0:2] != (2, 2):
        raise ValueError(f"A debe tener shape (2,2,Nx,offset), recibido {A_fp.shape}")
    ...
    for i in range(2):
        for j in range(2):
            ...
            out_path = os.path.join(out_dir, f"{prefix}_{i}{j}_compare.png")
            title = f"{prefix}[{i},{j}] : fp vs fxp vs diff"

            save_complex_compare_figure(X_fp, X_fxp, out_path, title)


def save_b_compare_figures(
    b_fp: np.ndarray,
    b_fxp: np.ndarray,
    out_dir: str,
    prefix: str = "b"
) -> None:
    if b_fp.shape != b_fxp.shape:
        raise ValueError(f"Shape mismatch: b_fp={b_fp.shape}, b_fxp={b_fxp.shape}")

    if b_fp.ndim != 3 or b_fp.shape[0] != 2:
        raise ValueError(f"b debe tener shape (2,Nx,offset), recibido {b_fp.shape}")

    for i in range(2):
        X_fp = b_fp[i, :, :]
        X_fxp = b_fxp[i, :, :]

        out_path = os.path.join(out_dir, f"{prefix}_{i}_compare.png")
        title = f"{prefix}[{i}] : fp vs fxp vs diff"
